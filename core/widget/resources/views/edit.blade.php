...
@section('content')
    <form action="{{ route('widgets.update', ['id' => $widget->id, 'lang_code' => $lang_code]) }}" method="POST">
        {{ csrf_field() }}
                ...
                <div class="m-portlet">
                    <div class="m-portlet__body">
                        @if($lang_code == get_default_lang_code())
                            <div class="form-group m-form__group row {{ $errors->has('positions') ? ' has-danger' : '' }}">
                                <label for="positions" class="col-2 col-form-label">
                                    Positions:
                                </label>
                                <div class="col-10">
                                    <select class="form-control m-input" id="positions" name="positions">
                                        <option value="{{$widget->positions}}">{{$widget->positions}}</option>
                                        @foreach(get_free_position('widget') as $position)
                            ...
                            <div class="col-lg-6">
                                <div class="dd" id="nestable">
                                    <ol id="h-widget" class="dd-list">
                                        {!! nestable_render_widget($widget->translate($lang_code)->content) !!}
                                    </ol>
                                </div>
