...
@section('title')
    Edit catalog "{{ $catalog->translate($lang_code)->the_title() }}"
@endsection

@section('content')
    <form action="{{ route('catalogs.update', ['id' => $catalog->id, 'lang_code' => $lang_code]) }}" method="POST">
        {{ csrf_field() }}
                ...
                <div class="m-portlet">
                    <div class="m-portlet__body">
                        <div class="form-group m-form__group row {{ $errors->has('title') ? ' has-danger' : '' }}">
                            ...
                            <div class="col-10">
                                <input class="form-control m-input" type="text" value="{{ $catalog->translate($lang_code)->the_title() }}" id="title" name="title" required>
                                @if ($errors->has('title'))
                        ...
                        <div class="form-group m-form__group row">
                            ...
                            <div class="col-10">
                                <input class="form-control m-input" type="text" value="{{ $catalog->translate($lang_code)->the_slug() }}" id="slug" name="slug" required>
                ...
                <div class="m-portlet m--margin-top-35">
                    ...
                    <div class="m-portlet__body">
                        <div class="form-group m-form__group row">
                            ...
                            <div class="col-10">
                                <textarea class="form-control m-input m-input--air" id="seo_title" name="seo_title" rows="3">{{ $catalog->seos($lang_code)->the_title() }}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="form-group m-form__group row">
                            ...
                            <div class="col-10">
                                <textarea class="form-control m-input m-input--air" id="seo_description" name="seo_description" rows="3">{{ $catalog->seos($lang_code)->the_description() }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="form-group m-form__group">
                                ...
                                <select class="form-control m-input" id="parent" name="parent">
                                    ...
                                    @foreach($catalogs as $cat)
                                        <option value="{{$cat->id}}" @if($catalog->parent_id == $cat->id) selected @endif>{{$cat->translate(get_default_lang_code())->title}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
